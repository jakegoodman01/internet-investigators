 ...
 * @version 3
 *
 * This class is the template for text screens in the game.
 * It uses the classes Type and Enter as sound affects and
...
public class TextScreen extends StackPane {
    ...
    public void paragraph(String[][] lineList, String[] lines, int delay, int paragraphNum){
        ...
        for (int i = 0; i<lineList[paragraphNum].length; i++) { // loops through the paragraph
            for (int j = 0;; j++) {
                ...
                {
                   /* timeline.getKeyFrames().addAll(new KeyFrame(Duration.millis(delay + 200 * (counter-1)), e -> t2.run(),
                            new KeyValue(boxes.get(paragraphNum).get(counter-1).translateXProperty(), 0)), new KeyFrame(Duration.millis(delay + 200 * (counter-1) + 1),
                                    new KeyValue(boxes.get(paragraphNum).get(counter-1).scaleXProperty(), 0)), new KeyFrame(Duration.millis(delay + 200 * counter + 500),
                ...
                if (lines[i].charAt(j) == ' ' && spaces == 0) { // if there is a space, but just one space in a row
                    spaces++;
                    timeline.getKeyFrames().add(new KeyFrame(Duration.millis(delay + 200 * counter), new KeyValue(boxes.get(paragraphNum).get(counter).scaleXProperty(), 1)));
                }
                else if (lines[i].charAt(j) != ' '){ // if there is a space, out of a series of spaces
                    spaces = 0;
                    /*timeline.getKeyFrames().add(new KeyFrame(Duration.millis(delay + 200 * counter), e -> t1.run(),
                            new KeyValue(boxes.get(paragraphNum).get(counter).scaleXProperty(), 1)));*/
                }

                if (spaces < 1) { // if the character is not a space
                    timeline.getKeyFrames().addAll(new KeyFrame(Duration.millis(delay + 200 * counter + 1),
                            new KeyValue(boxes.get(paragraphNum).get(counter).scaleXProperty(), 0)), new KeyFrame(Duration.millis(delay + 200 * (counter + 1)),
    ...
    /**
     * Plays the enter sound file based on the amound of spaces between paragraphs.
     * @param lines
     */
    public void newParagraph(int lines)
    {
        ...
        for (int i = 0; i<lines; i++) {
            //timeline.getKeyFrames().add(new KeyFrame(Duration.millis(i*500), e -> t2.run())); // makes the enter sound of a typewriter
        }
        timeline.play(); // playes the sound
    }
